...
class Conversation {
    ...
    public function create(int $id_user1, int $id_user2): int {
        ...
        if ($existing) return (int)$existing['id_conversation'];

        // Si la table ne possède pas de colonne AUTO_INCREMENT,
        // on calcule une nouvelle clé primaire manuellement.
        $stmt = $this->pdo->query("SELECT MAX(id_conversation) AS max_id FROM conversations");
        $maxId = (int)$stmt->fetchColumn();
        $newId = $maxId + 1;

        $stmt = $this->pdo->prepare(
            "INSERT INTO conversations (id_conversation, id_user1, id_user2) VALUES (:id, :u1, :u2)"
        );
        $stmt->execute([':id' => $newId, ':u1' => $id_user1, ':u2' => $id_user2]);
        return $newId;
    }
