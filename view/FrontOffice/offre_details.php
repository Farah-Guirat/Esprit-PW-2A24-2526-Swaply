    ...
    <div class="p-8">
      ...
      <div class="flex gap-3 pt-6 border-t border-gray-200">
        ...
        <?php if ($offre->getIdU() == 2): ?>
          ...
          <button onclick="document.getElementById('deleteModal').classList.remove('hidden')" class="inline-flex items-center gap-2 bg-red-500 text-white px-6 py-3 rounded-xl hover:bg-red-600 transition font-medium shadow-md">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
...
<script>
  // Seulement 2 fonctions simples pour le modal
...
<script>
  ...
  function generateQR() {
    try {
      ...
      // Créer le contenu textuel concis avec les détails essentiels
   const url = "http://localhost/swaply/public/index.php?action=show&id=" + offerData.id;
   const offerDetails =
"SWAPLY\n" +
"Titre: " + offerData.titre + "\n" +
"Statut: " + offerData.statut + "\n" +
"Date limite: " + offerData.dateLimite + "\n\n" +
url;
      // Create QR code with offer details
      new QRCode(qrBox, {
  text: offerDetails,
  width: 150,
  height: 150,
